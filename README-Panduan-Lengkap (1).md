# Siaga Darurat — Panduan Lengkap Setup & Hosting (untuk Pemula)
...
- [Bagian 1 — Buat Database Gratis (Supabase)](#bagian-1--buat-database-gratis-supabase)
...
## Bagian 1 — Buat Database Gratis (Supabase)

Database ini tempat semua data (akun, order, chat, rating) benar-benar
tersimpan secara permanen.

### 1.1 Daftar di Supabase

1. Buka **https://supabase.com**
2. Klik tombol **Start your project** atau **Sign Up** di pojok kanan atas.
3. Pilih daftar pakai **GitHub** (lebih cepat, tidak perlu verifikasi email
   terpisah).
4. Setelah login, Anda akan diarahkan ke halaman dashboard Supabase.

### 1.2 Buat Project Baru

1. Klik tombol **New Project**.
2. Kalau diminta pilih organisasi, pilih yang otomatis dibuat dengan nama
   akun Anda (atau buat baru kalau diminta).
3. Isi:
   - **Name**: `siaga-darurat` (atau nama lain sesuai keinginan)
   - **Database Password**: buat password yang kuat, lalu **simpan di
     Notepad** — akan dibutuhkan lagi nanti (Supabase tidak menampilkannya
     lagi setelah ini)
   - **Region**: pilih yang paling dekat, misalnya **Southeast Asia
     (Singapore)** — supaya akses dari Indonesia lebih cepat
4. Klik **Create new project**. Tunggu 1-2 menit sampai database selesai
   disiapkan (ada indikator progress di layar).

### 1.3 Ambil Connection String

1. Setelah project selesai dibuat, klik ikon **⚙️ Settings** di sidebar kiri
   bawah, lalu klik **Database**.
2. Scroll ke bagian **Connection string**, lalu pilih tab **URI**.
3. Anda akan melihat connection string seperti ini:
   ```
   postgresql://postgres:[YOUR-PASSWORD]@db.xxxxxxxxxxxx.supabase.co:5432/postgres
   ```
4. Klik ikon **copy** di sebelah kotak tersebut untuk menyalin seluruh teks
   ini.
5. **Ganti bagian `[YOUR-PASSWORD]`** dengan password database yang Anda
   buat di langkah 1.2 (termasuk tanda kurung sikunya dihapus, hanya isi
   passwordnya saja yang dimasukkan).
6. **Tempelkan (paste) connection string ini ke Notepad atau catatan
   sementara** — kita akan memakainya beberapa kali di langkah-langkah
   berikutnya. Jangan sampai hilang.

> 💡 **Kelebihan Supabase**: selain databasenya gratis dan lebih besar
> kuotanya dibanding beberapa layanan sejenis, Supabase juga punya
> **Table Editor** (klik menu **Table Editor** di sidebar) — tampilan
> seperti spreadsheet untuk melihat langsung isi data users, orders, chat,
> dll tanpa perlu perintah SQL sama sekali.
...
4. Anda akan melihat isinya seperti ini:
   ...
   PORT=3000
   ```
5. **Ganti baris `DATABASE_URL=`** dengan connection string Supabase yang sudah
...
segera ganti `JWT_SECRET` serta password database Anda di Supabase (Settings → Database → Reset Database Password).
...
| Name | Value |
|---|---|
| `DATABASE_URL` | connection string Supabase Anda (yang sama seperti di Bagian 1.3) |
| `JWT_SECRET` | string acak yang sama seperti yang Anda buat di Bagian 2.3 |

Caranya:
1. Di kolom **Name**, ketik `DATABASE_URL`.
2. Di kolom **Value**, paste connection string Supabase Anda.
3. Klik **Add**.
...
sebelumnya dengan `DATABASE_URL` Supabase yang sama, **tabel sudah otomatis ada**
juga untuk versi Vercel — karena keduanya memakai database Supabase yang sama
...
2. Pastikan file `.env` sudah ada dan `DATABASE_URL` sudah diisi connection
   string Supabase (ikuti Bagian 2.3 kalau belum).
3. Jalankan:
   ```
   npm install
   npm run db:init
...
- **Biaya**: Vercel free tier dan Supabase free tier cukup untuk skripsi, demo,
...
- **Pesanan menunggu teknisi asli, bukan otomatis**: saat pelanggan yang
  sudah login menekan "Cari Teknisi", pesanan akan **menunggu sungguhan**
  sampai ada teknisi yang login dan menekan tombol **Terima** di menu
  "Pesanan Masuk" miliknya. Tidak ada lagi teknisi contoh/dummy yang
  otomatis "menerima" pesanan setelah beberapa detik. Kalau belum ada
  teknisi yang online untuk menerima, pelanggan akan tetap melihat status
  "Menunggu teknisi tersedia..." — dan bisa menekan tombol **Batal** kapan
  saja kalau ingin membatalkan. (Simulasi otomatis hanya berjalan untuk
  pengunjung yang **belum login**, sebagai contoh tampilan demo saja.)
...
- Cek lagi isi file `.env`, pastikan `DATABASE_URL` benar-benar disalin
  utuh dari Supabase, dan `[YOUR-PASSWORD]` sudah diganti dengan password asli.
- Pastikan tidak ada spasi tambahan sebelum/sesudah connection string.

**Error "password authentication failed" saat `db:init`:**
- Connection string mungkin salah disalin, atau password di connection string
  belum diganti dari `[YOUR-PASSWORD]`. Kembali ke Supabase Dashboard →
...
- Pastikan juga Bagian 5 (buat tabel) sudah dijalankan dengan
  `DATABASE_URL` Supabase yang **sama** dengan yang dipakai di Vercel.
